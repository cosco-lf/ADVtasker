...
if (@$_REQUEST['action'] == 'login') {
  if (isset($_POST['rlogin']) && isset($_POST['rpass'])) {
    ...
    if (existThis('tasker_users','login = "'.sql_quote($rlogin).'" AND pass = "'.sql_quote($rpass).'" AND rights>0 AND remove<=0')) {
...
if (@$_REQUEST['action'] == 'remindme') {
  if (isset($_POST['rEmail'])) {
    $remind = @selectAll('tasker_users',4,'*,sha1(`pass`) as url','email="'.sql_quote($_POST['rEmail']).'" OR login="'.sql_quote($_POST['rEmail']).'"');
    if (is_array(@$remind)) {
      $remind['site'] = $siteurl;
...
if (isset($_SESSION['login']) && isset($_SESSION['pass'])) {
  $user = selectAll('tasker_users',4,'*','`login` = "'.$_SESSION['login'].'"');
  if (is_array($user)) {
    $isLogin = true;
  }
}
...
//todo рубрикатор очень прост нет вложености
        ...
        $page = selectAll('tasker_rubrics',4,'*','url="'.$urlParts[1].'"');
        if (!$ajax) {
          include($header);
        }
        include($mainTpl);
